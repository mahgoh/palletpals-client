...
export default function AdminWarehouseCreate() {
  ...
  const { data = {}, error, loading } = API.Warehouse.byId(warehouseId)
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        await API.Warehouse.patch(warehouseId, payload)
        showNotification(t('message.warehouse-updated'))
        navigate('/admin/warehouses')
      } catch (e) {
        showNotification(t('message.warehouse-not-updated'))
        console.error(e)
      }
    },
  })
  ...
  return (
    <>
      <Main>
        <Pagetitle title={t('common.warehouse.edit')} />
        <Spacer size="lg" />
        {renderForm()}
      </Main>
    </>
